...
| 检查 | 结果 |
|---|---|
| 核心调度、缓存、采样语义 | 15 项核心测试与 7 项前端/HTTP/UTF-8 测试通过（启用 llama feature 共 22 项） |
...
最终 SmolLM2 流式断连测试启用 --require-cancel，实际观测 total_cancelled 增加 1，活动/排队/预留资源全部归零。cargo fmt --check、默认与 llama feature 的 cargo clippy --all-targets -- -D warnings 均通过；默认构建 18 项测试、llama 构建 22 项测试通过。
