...
参考官方 mini-sglang 的 C++17 教学推理运行时：原生调度、压缩 radix 前缀缓存、分块 prefill、连续批处理和请求级采样，使用 llama.cpp 执行真实 GGUF 模型。HTTP 支持文本补全、基于模型模板的聊天补全和 SSE 流式输出。
...
需要 C++17 编译器、CMake 3.24+、Git。默认只构建核心库与语义测试，不下载模型或 llama.cpp：
