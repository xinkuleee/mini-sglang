...
class Engine {
  public:
    ...
    // batch 仅在同步调用期间借用；后端不得保留 span。返回值拥有 logits 数据，
    // 只包含 logits=true 的行，顺序与 batch 中的行相同。
    virtual std::vector<std::vector<float>> forward(std::span<const BatchToken> batch) = 0;
